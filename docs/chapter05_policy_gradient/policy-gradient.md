...
DQN 的策略是确定性的（永远选 $\arg\max$），探索必须靠外部手段——第 4 章用的是 $\varepsilon$-greedy：以概率 $\varepsilon$ 随机选动作，否则选 $Q$ 值最大的。$\varepsilon$ 需要手动调度，太大浪费，太小不够探索（回顾：[DQN 的三个组件](../chapter04_dqn/dqn-components)）。
...
这是两条路线最实际的工程差异。DQN 是 **off-policy** 的：经验回放池里存着旧数据，可以反复拿来训练（回顾：[经验回放](../chapter04_dqn/dqn-components)）。一条经验可以被多个 batch 复用，数据效率高。
...
REINFORCE 能工作，但"高方差"让它几乎不可用。好在策略梯度定理有一个奇妙的性质：你可以在梯度估计中减去一个不依赖于动作的"基线"，既不改变梯度的期望方向，又能大幅降低方差。这个发现直接导向了 Actor-Critic 架构。让我们在下一章——[Actor-Critic 架构](../chapter06_actor_critic/actor-critic)——中看看它是怎么做到的。
